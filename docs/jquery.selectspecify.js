...
jQuery.fn.selectSpecify = function(options){
	...
	$(selector).replaceWith($('<div />').addClass(mainClass).addClass('select-specify').append());
	...
	$(selector + ' .input-bar').append($('<span />').addClass('value').append(settings.itemLabelText + ': <br />').
		...
		).append('<br />').append($('<a />').attr('href', 'javascript:void(0);').addClass('add ' + settings.addButtonClass).html(settings.addButtonText))).append(' ');
